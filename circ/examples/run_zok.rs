...
fn main() {
    ...
    // test_dot_chacha_batching();
    // test_aes();
    // test_doh_aes();
    
    // test_precomp_doh_chacha();
    // test_unpack_doh_chacha();

    // test_precomp_dot_chacha();
    // test_unpack_dot_chacha();
    
    // test_precomp_doh_aes();
    test_unpack_doh_aes();
...
impl ZKMBLogger {
    fn default(name: String) -> ZKMBLogger {
        let mut file = File::create(name.clone()).unwrap();
        ZKMBLogger { name: name.clone(), logger: "file".to_string(), file_name: name.to_string() }
    }

    fn new(name: String, logger: String, file_name: String) -> ZKMBLogger {
        if logger != "console" {
            let mut file = File::create(file_name.clone()).unwrap();
        }
        ZKMBLogger { name: name.clone(), logger: logger.clone(), file_name: file_name.clone() }
    }

    fn print(&self, str: String) {
        if self.logger == "console" {
            println!("{}", str);
        } else if self.logger == "file" {
...
fn test_sbox() {
    ...
    new_evaluate_cs(cs, &input_map, ZKMBLogger::default("TEST_SBOX".to_string()));
}

fn test_precomp_doh_chacha() {
    let inputs = Inputs {
        file: PathBuf::from("./zkmb/PrecompDohChaCha.zok"),
        mode: Mode::Proof,
        isolate_asserts: true,
    };
    ...
    let mut input_map = HashMap::<String, Value>::default();
    map_precomp_chacha(&mut input_map, &key, &nonce, comm, SN);
    
    println!("starting evaluation");
    // let assignment = evaluate_cs(cs, &input_map);
    // print_u8_arr_result(&assignment);
    new_evaluate_cs(cs, &input_map, ZKMBLogger::default("PrecompDohChaCha".to_string()));
...
fn test_unpack_doh_chacha() {
    let inputs = Inputs {
        file: PathBuf::from("./zkmb/UnpackDohChaCha.zok"),
        mode: Mode::Proof,
        isolate_asserts: true,
    };
    println!("starting gen");
...
fn test_precomp_dot_chacha() {
    let inputs = Inputs {
        file: PathBuf::from("./zkmb/PrecompDotChaCha.zok"),
        mode: Mode::Proof,
        isolate_asserts: true,
    };
    println!("starting gen");
    ...
    let mut input_map = HashMap::<String, Value>::default();
    map_precomp_chacha(&mut input_map, &key, &nonce, comm, SN); // exact same mapping function
    
    println!("starting evaluation");
    // let assignment = evaluate_cs(cs, &input_map);
    // print_u8_arr_result(&assignment);
    new_evaluate_cs(cs, &input_map, ZKMBLogger::default("PrecompDotChaCha".to_string()));
    // print_return(&assignment);
    // println!("will generate r1cs");
    // let (r1cs, prover_data, verifier_data) = to_r1cs(cs, FieldT::from(FieldT::from(DFL_T.modulus())));
    // println!("r1cs timer {}", timer.elapsed().as_millis());
...
fn test_unpack_dot_chacha() {
    let inputs = Inputs {
        file: PathBuf::from("./zkmb/UnpackDotChaCha.zok"),
        mode: Mode::Proof,
        isolate_asserts: true,
    };
    println!("starting gen");
    ...
    let mut input_map = HashMap::<String, Value>::default();
    map_unpack_doh_chacha(&mut input_map, key, nonce, comm, SN, packed_pad, comm_pad);
    
    println!("starting evaluation");
    // let assignment = evaluate_cs(cs, &input_map);
    // print_u8_arr_result(&assignment);
    // new_evaluate_cs(cs, &input_map, ZKMBLogger::default("UnpackDohChaCha".to_string()));
    just_get_constraint_count(cs, &input_map, ZKMBLogger::default("UnpackDotChaCha".to_string()));
    // print_return(&assignment);
    // println!("will generate r1cs");
    // let (r1cs, prover_data, verifier_data) = to_r1cs(cs, FieldT::from(FieldT::from(DFL_T.modulus())));
    // println!("r1cs timer {}", timer.elapsed().as_millis());
...
fn test_precomp_doh_aes() {
    let inputs = Inputs {
        file: PathBuf::from("./zkmb/PrecompDohAES.zok"),
        mode: Mode::Proof,
        isolate_asserts: true,
    };
    println!("starting gen");
    ...
    let mut input_map = HashMap::<String, Value>::default();
    map_precomp_aes(&mut input_map, &key, &iv, comm_hex, SN);
    
    println!("starting evaluation");
    new_evaluate_cs(cs, &input_map, ZKMBLogger::default("PrecompDohAES".to_string()));
}

fn test_unpack_doh_aes() {
    let inputs = Inputs {
        file: PathBuf::from("./zkmb/UnpackDohAES.zok"),
        mode: Mode::Proof,
        isolate_asserts: true,
    };
    println!("starting gen");
    ...
    let mut input_map = HashMap::<String, Value>::default();
    map_unpack_doh_aes(&mut input_map, key, iv, comm_hex, SN, packed_pad, comm_pad);
    
    println!("starting evaluation");
    // let assignment = evaluate_cs(cs, &input_map);
    // print_u8_arr_result(&assignment);
    // new_evaluate_cs(cs, &input_map, ZKMBLogger::default("UnpackDohChaCha".to_string()));
    just_get_constraint_count(cs, &input_map, ZKMBLogger::default("UnpackDohAES".to_string()));
    // print_return(&assignment);
    // println!("will generate r1cs");
    // let (r1cs, prover_data, verifier_data) = to_r1cs(cs, FieldT::from(FieldT::from(DFL_T.modulus())));
    // println!("r1cs timer {}", timer.elapsed().as_millis());
...
fn test_aes() {
    ...
    // let assignment = evaluate_cs(cs, &input_map);
    // print_u8_arr_result(&assignment);
    new_evaluate_cs(cs, &input_map, ZKMBLogger::default("AES".to_string()));
}


fn test_shaRound() {
    let inputs = Inputs {
        file: PathBuf::from("./zkmb/shaRound.zok"),
        mode: Mode::Proof,
        isolate_asserts: true,
    };
    let cs = ZSharpFE::gen(inputs);
    ...
    new_evaluate_cs(cs, &input_map, ZKMBLogger::default("shaRound".to_string()));
    // let assignment = evaluate_cs(cs, &input_map);
}
...
fn test_tls_session_setup() {
    let (HS, H2, CH_SH_len, ServExt_len, ServExt_ct_tail_padded, ServExt_tail_len, SHA_H_Checkpoint, comm) = read_dot_chacha_co_json("./zkmb/tls.json");
    let inputs = Inputs {
        file: PathBuf::from("./zkmb/TLSSessionSetup.zok"),
        mode: Mode::Proof,
        isolate_asserts: true,
    };
    let timer = Instant::now();
    ...
    let mut input_map = HashMap::<String, Value>::default(); 
    map_dot_chacha_co(&mut input_map, HS, H2, CH_SH_len, ServExt_len, ServExt_ct_tail_padded, ServExt_tail_len, SHA_H_Checkpoint, comm);
    new_evaluate_cs(cs, &input_map, ZKMBLogger::default("TLSSessionSetup".to_string()));
}
...
fn test_doh_aes() {
    ...
    // let assignment = evaluate_cs(cs, &input_map);
    // print_u8_arr_result(&assignment);
    new_evaluate_cs(cs, &input_map, ZKMBLogger::default("DohAES".to_string()));
    // print_return(&assignment);
    // println!("will generate r1cs");
    // let (r1cs, prover_data, verifier_data) = to_r1cs(cs, FieldT::from(FieldT::from(DFL_T.modulus())));
    // println!("r1cs timer {}", timer.elapsed().as_millis());
...
fn test_doh_chacha() {
    ...
    // let assignment = evaluate_cs(cs, &input_map);
    // print_u8_arr_result(&assignment);
    new_evaluate_cs(cs, &input_map, ZKMBLogger::default("DohChaCha".to_string()));
    // print_return(&assignment);
    // println!("will generate r1cs");
    // let (r1cs, prover_data, verifier_data) = to_r1cs(cs, FieldT::from(FieldT::from(DFL_T.modulus())));
    // println!("r1cs timer {}", timer.elapsed().as_millis());
...
fn test_dot_chacha() {
    ...
    new_evaluate_cs(cs, &input_map, ZKMBLogger::default("DotChaCha".to_string()));
...
fn map_precomp_chacha(input_map: &mut HashMap::<String, Value>, key: &[u8], nonce: &[u8], comm: &str, SN: u32) {
    map_u8_arr(&key, "key", input_map);
    map_u8_arr(&nonce, "nonce", input_map);
    input_map.insert("comm".to_string(), str_to_field(comm));
    input_map.insert("SN".to_string(), u32_to_value(SN));
...
fn map_precomp_aes(input_map: &mut HashMap::<String, Value>, key: &[u8], iv: &[u8], comm: &str, SN: u32) {
    map_u8_arr(&key, "key", input_map);
    map_u8_arr(&iv, "iv", input_map);
    input_map.insert("comm".to_string(), hex_str_to_field(comm));
    input_map.insert("SN".to_string(), u32_to_value(SN));
...
fn map_unpack_doh_chacha(input_map: &mut HashMap::<String, Value>, key: [u8; 32], nonce: [u8; 12], comm: &str, SN: u32, packed_pad: Vec<&str>, comm_pad: &str) {
    map_u8_arr(&key, "key", input_map);
    map_u8_arr(&nonce, "nonce", input_map);
    input_map.insert("comm".to_string(), str_to_field(comm));
    input_map.insert("SN".to_string(), u32_to_value(SN));
    for (i, s) in packed_pad.iter().enumerate() {
        input_map.insert(format!("packed_pad.{}", i), str_to_field(s));
    }
    input_map.insert("comm_pad".to_string(), str_to_field(comm_pad));
}

fn map_unpack_doh_aes(input_map: &mut HashMap::<String, Value>, key: [u8; 16], iv: [u8; 12], comm: &str, SN: u32, packed_pad: Vec<&str>, comm_pad: &str) {
    map_u8_arr(&key, "key", input_map);
    map_u8_arr(&iv, "iv", input_map);
    input_map.insert("comm".to_string(), hex_str_to_field(comm));
    input_map.insert("SN".to_string(), u32_to_value(SN));
...
fn test_non_membership() {
    ...
    new_evaluate_cs(cs, &input_map, ZKMBLogger::default("NonMembership".to_string()));
}
...
fn new_evaluate_cs(cs: Computation, input_map: &HashMap::<String, Value>, logger: ZKMBLogger) {
    let timer = Instant::now();
    println!("will generate r1cs");
    let (r1cs, prover_data, verifier_data) = to_r1cs(cs, FieldT::from(FieldT::from(DFL_T.modulus())));
    let r1cs = reduce_linearities(r1cs, Some(50));
    println!("r1cs timer {}", timer.elapsed().as_millis());
    logger.print(format!("r1cs constraints {}", r1cs.constraints().len()));
    let precomp = precomp_opt(
        prover_data.precompute,
        vec![
            Opt::ConstantFold(Box::new([])),
    ...
    let t5 = Instant::now();
    logger.print(format!("NIZK proof took {}", t5.duration_since(t4).as_millis()));
    ...
    logger.print(format!("NIZK verify took {} {}", timer.elapsed().as_millis(), ok)); 
}

fn just_get_constraint_count(cs: Computation, input_map: &HashMap::<String, Value>, logger: ZKMBLogger) {
    let timer = Instant::now();
    println!("will generate r1cs");
    let (r1cs, prover_data, verifier_data) = to_r1cs(cs, FieldT::from(FieldT::from(DFL_T.modulus())));
    let r1cs = reduce_linearities(r1cs, Some(50));
...
fn benchmark_batching(cs: Computation, input_map: &HashMap::<String, Value>, logger: ZKMBLogger, batch_size: usize) {
    let timer = Instant::now();
    println!("will generate r1cs");
    let (r1cs, prover_data, verifier_data) = to_r1cs(cs, FieldT::from(FieldT::from(DFL_T.modulus())));
    let r1cs = reduce_linearities(r1cs, Some(50));
    ...
    // actuall proof start
    let mut prover_transcript = Transcript::new(b"zkmb_proof");
    let vars_list = vec![var_assignment.clone(); batch_size];
    let inputs_list = vec![input_assignment.clone(); batch_size];
    let t4 = Instant::now();
    let proof = NIZK::prove(
      &inst,
      &vars_list,
    ...
    let t5 = Instant::now();
    logger.print(format!("batch {} NIZK proof took {}", batch_size, t5.duration_since(t4).as_millis()));

    let timer = Instant::now();
    let mut verifier_transcript = Transcript::new(b"zkmb_proof");
    let ok = proof.verify(&inst, &inputs_list, &mut verifier_transcript, &gens).is_ok();
    logger.print(format!("batch {} NIZK verify took {} {}", batch_size, timer.elapsed().as_millis(), ok)); 
}

fn test_dot_chacha_batching() {
    let inputs = Inputs {
        file: PathBuf::from("./zkmb/DotChaChaAmortized.zok"),
        mode: Mode::Proof,
        isolate_asserts: true,
    ...
    let mut input_map = HashMap::<String, Value>::default();
    map_dot_chacha(&mut input_map, key, nonce, comm, SN, &dns_ct, input_domain_name_wildcard, root, left_domain_name, right_domain_name, left_index, right_index, left_path_array, right_path_array, left_dir, right_dir);
    benchmark_batching(cs.clone(), &input_map, ZKMBLogger::default("DotChaCha_batch1".to_string()), 1);
    benchmark_batching(cs.clone(), &input_map, ZKMBLogger::default("DotChaCha_batch5".to_string()), 5);
    benchmark_batching(cs.clone(), &input_map, ZKMBLogger::default("DotChaCha_batch10".to_string()), 10);
    benchmark_batching(cs.clone(), &input_map, ZKMBLogger::default("DotChaCha_batch20".to_string()), 20);
    benchmark_batching(cs.clone(), &input_map, ZKMBLogger::default("DotChaCha_batch50".to_string()), 50);
}