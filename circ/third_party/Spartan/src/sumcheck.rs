...
impl ZKSumcheckInstanceProof {
  ...
  pub fn batch_verify(
    ...
    for i in 0..self.comm_polys.len() {
      ...
      comm_poly.append_to_transcript(b"comm_poly", &mut transcript.trans);

      //derive the verifier's challenge for the next round
      // current_transcripts don't have to be cleared, since we can make sure it is updated by all the threads each time
      let mut transcripts = current_transcripts.lock().unwrap();
      transcripts[idx] = transcript.trans.clone();
      drop(transcripts);
      // println!("before barrier");
      barrier.wait();
      // println!("after barrier");
      let mut transcripts = current_transcripts.lock().unwrap().clone();
      let mut new_transcript = Transcript::new(b"zkmb_proof");
      for t in transcripts {
        let s = t.clone().challenge_scalar(b"combine");
        new_transcript.append_scalar(b"combine", &s);
      }
      transcript.trans = new_transcript;
            
      let r_i = transcript.trans.challenge_scalar(b"challenge_nextround");
      barrier.wait();
      ...
      let res = {
        ...
        comm_claim_per_round.append_to_transcript(b"comm_claim_per_round", &mut transcript.trans);
        comm_eval.append_to_transcript(b"comm_eval", &mut transcript.trans);

        // produce two weights
        let w = transcript.trans.challenge_vector(b"combine_two_claims_to_one", 2);
...
impl ZKSumcheckInstanceProof {
    ...
    F: Fn(&Scalar, &Scalar) -> Scalar + Sync,
  {
    ...
    for j in 0..num_rounds {
      ...
      //derive the verifier's challenge for the next round
      let r_j = transcript.trans.challenge_scalar(b"challenge_nextround");
      // println!("round {} idx {} {:?}", j, idx, r_j);
      // prevent current_transcripts being updated by next round
      barrier.wait();
  ...
  pub fn batch_prove_cubic_with_additive_term<F>(    
    claim: &Scalar,
    blind_claim: &Scalar,
    num_rounds: usize,
    poly_A: &mut DensePolynomial,
