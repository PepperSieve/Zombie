...
// thread_local! {
//   static rx_BUF: std::cell::RefCell<Vec<Scalar>> = std::cell::RefCell::new(vec![Scalar::zero(); 131072 * 4]);
//   static ry_BUF: std::cell::RefCell<Vec<Scalar>> = std::cell::RefCell::new(vec![Scalar::zero(); 131072 * 4]);
// }
...
impl SparseMatPolynomial {
  ...
  pub fn multi_evaluate_test(
    polys: &[&SparseMatPolynomial],
    rx: &[Scalar],
    ry: &[Scalar],
  ) -> Vec<Scalar> {
    // rx_BUF.with(|rx_buf| {
    //   let mut buffer = rx_buf.borrow_mut();
    //   let timer = std::time::Instant::now();
    //   let poly = EqPolynomial::new(rx.to_vec());
    //   // println!("poly alloc rx takes {}", timer.elapsed().as_micros());
    //   let evals_len = poly.evals_test(&mut *buffer);
    //   let eval_table_rx = &mut buffer[0..evals_len];
    //   ry_BUF.with(|ry_buf| {
    //     let mut buffer = ry_buf.borrow_mut();
    //     let timer = std::time::Instant::now();
    //     let poly = EqPolynomial::new(ry.to_vec());
    //     // println!("poly alloc ry takes {}", timer.elapsed().as_micros());
    //     let evals_len = poly.evals_test(&mut *buffer);
    //     let eval_table_ry = &mut buffer[0..evals_len];
    //     // let eval_table_ry = EqPolynomial::new(ry.to_vec()).evals_test();
    //     (0..polys.len())
    //       .map(|i| polys[i].evaluate_with_tables(&*eval_table_rx, &*eval_table_ry))
    //       .collect::<Vec<Scalar>>()
    //   })
