...
impl MyTimer {
  ...
  pub fn stop(&self) {
    println!("{} takes {}ms", self.label, self.start.elapsed().as_millis())
  }
}
...
impl R1CSProof {
  fn batch_prove_phase_one(
    ...
    transcript: &mut TranscriptWrapper,
    random_tape: &mut RandomTape,
    barrier: Arc<Barrier>,
    current_transcripts: Arc::<Mutex::<Vec<Transcript>>>,
    idx: usize
    ...
    let (sc_proof_phase_one, r, claims, blind_claim_postsc) =
      ZKSumcheckInstanceProof::batch_prove_cubic_with_additive_term(
  ...
  fn batch_prove_phase_two(
    ...
    let (sc_proof_phase_two, r, claims, blind_claim_postsc) = ZKSumcheckInstanceProof::batch_prove_quad(
      claim,
      blind_claim,
      num_rounds,
      evals_z,
  ...
  pub fn batch_prove(
    ...
    transcript.trans.append_protocol_name(R1CSProof::protocol_name());
    ...
    let (poly_vars, comm_vars, blinds_vars) = {
      ...
      comm_vars.append_to_transcript(b"poly_commitment", &mut transcript.trans);
      (poly_vars, comm_vars, blinds_vars)
    };
    // println!("commitment time is {}", t1.elapsed().as_millis());
    // barrier.wait();
    timer_commit.stop();
    ...
    let tau = transcript.trans.challenge_vector(b"challenge_tau", num_rounds_x);
    // compute the initial evaluation table for R(\tau, x)
    let mut poly_tau = DensePolynomial::new(EqPolynomial::new(tau).evals());
    let (mut poly_Az, mut poly_Bz, mut poly_Cz) =
      inst.multiply_vec(inst.get_num_cons(), z.len(), &z);

    let (sc_proof_phase1, rx, _claims_phase1, blind_claim_postsc1) = R1CSProof::batch_prove_phase_one(
      num_rounds_x,
      &mut poly_tau,
      &mut poly_Az,
      &mut poly_Bz,
    ...
    comm_Az_claim.append_to_transcript(b"comm_Az_claim", &mut transcript.trans);
    comm_Bz_claim.append_to_transcript(b"comm_Bz_claim", &mut transcript.trans);
    comm_Cz_claim.append_to_transcript(b"comm_Cz_claim", &mut transcript.trans);
    comm_prod_Az_Bz_claims.append_to_transcript(b"comm_prod_Az_Bz_claims", &mut transcript.trans);
    ...
    // let t1 = MyTimer::new("second sum check".to_string());
    let timer_sc_proof_phase2 = Timer::new("prove_sc_phase_two");
    // combine the three claims into a single claim
    let r_A = transcript.trans.challenge_scalar(b"challenege_Az");
    let r_B = transcript.trans.challenge_scalar(b"challenege_Bz");
    let r_C = transcript.trans.challenge_scalar(b"challenege_Cz");
    ...
    let (sc_proof_phase2, ry, claims_phase2, blind_claim_postsc2) = R1CSProof::batch_prove_phase_two(
      num_rounds_y,
      &claim_phase2,
      &blind_claim_phase2,
    ...
    // println!("decommitment time is {}", t1.elapsed().as_millis());
    // barrier.wait();
    timer_polyeval.stop();
  ...
  pub fn batch_verify(
    ...
    self
      .comm_vars
      .append_to_transcript(b"poly_commitment", &mut transcript.trans);
    ...
    let claim_phase1 = Scalar::zero()
      ...
      .compress();
    let (comm_claim_post_phase1, rx) = self.sc_proof_phase1.batch_verify(
      &claim_phase1,
      num_rounds_x,
      3,
      &gens.gens_sc.gens_1,
    ...
    assert!(pok_Cz_claim
      .verify(&gens.gens_sc.gens_1, &mut transcript.trans, &comm_Cz_claim)
      .is_ok());
    assert!(proof_prod
      .verify(
        &gens.gens_sc.gens_1,
    ...
    let (comm_claim_post_phase2, ry) = self.sc_proof_phase2.batch_verify(
      &comm_claim_phase2,
      num_rounds_y,
      2,
      &gens.gens_sc.gens_1,
