...
impl EqPolynomial {
  ...
  pub fn evals(&self) -> Vec<Scalar> {
    ...
    */
    ...
    for r in self.r.iter().rev() {
      ...
      let iter = evals_left.iter_mut().zip(evals_right.iter_mut());
      // copy each element from the prior iteration twice
      iter.for_each(|(x, y)| {
        *y = *x * r;
        *x -= &*y;
  ...
  pub fn evals_test(&self, buffer: &mut Vec<Scalar>) -> usize {
      let ell = self.r.len();
      let evals_len = ell.pow2();
      // println!("{} {}", buffer.len(), evals_len);
      assert!(buffer.len() >= evals_len);
      let evals = &mut buffer[0..evals_len];
      for idx in 0..evals.len() {
        evals[idx] = Scalar::zero();
      }
      let mut size = 1;
      evals[0] = Scalar::one();
      for r in self.r.iter().rev() {
      ...
      // let res = evals.to_owned().into_boxed_slice();
      // println!("evals to box takes {}", timer.elapsed().as_micros());
      // res
      evals_len
  }
