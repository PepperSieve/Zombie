...
impl<'de> Deserialize<'de> for NumTerm {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        const FIELDS: &'static [&'static str] = &["op", "cs"];
...
impl<'de> Deserialize<'de> for Array {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        const FIELDS: &'static [&'static str] = &["key_sort", "default", "map", "size"];
...
pub fn eval_value_efficient(
    ...
    val_arr: &mut Vec<Option<Value>>,
    h: &FxHashMap<String, Value>,
) {
    let c = &term_arr[term_idx];
    let v: Value = match &c.op {
        NumOp::Var(idx) => val_arr[idx.clone()].clone().unwrap(),
        NumOp::Op(op) => {
            match op {
                Op::Var(n, _) => {
                    let v = h
                ...
                Op::BvConcat => Value::BitVector({
                    ...
                    it.fold(f.clone(), BitVector::concat)
                }),
